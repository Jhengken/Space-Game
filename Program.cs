    ...
    private static async Task Main(string[] args)
    {
        ...
        do
        {
            ...
            if (!stop)
            {
                ...
                p.WriteAt("STOP", mapX / 2 - 2, mapY / 2 + 1);
                p.WriteAt("STOP", mapX / 2 - 2, mapY / 2);
                p.WriteAt("STOP", mapX / 2 - 2, mapY / 2 - 1);
                Console.ForegroundColor = ConsoleColor.White;
                if (Console.ReadKey().Key == ConsoleKey.Spacebar)
                {
                    p.WriteAt("    ", mapX / 2 - 2, mapY / 2 + 1);
                    p.WriteAt("    ", mapX / 2 - 2, mapY / 2);
                    p.WriteAt("    ", mapX / 2 - 2, mapY / 2 - 1);
                    ConsoleMeteorite();
                    StarShipConsole();
                    stop = true;
        ...
        p.WriteAt("Game Over", mapX / 2 - 4, mapY / 2 - 2);
        p.WriteAt("And", mapX / 2 - 1, mapY / 2);
        p.WriteAt("Press Enter", mapX / 2 - 5, mapY / 2 + 2);
        Console.ForegroundColor = ConsoleColor.White;
                    ...
                    else
                    {
                        if (strMap2D[y, x] == " ")
                            if (strMap2D[y + 1, x] == "X" | strMap2D[y + 1, x] == "*")
                                p.WriteAt(" ", x, y);   //下面有隕石或邊界就給他空白
                            else
                            { }  //空白就不用印了
                        else
                            p.WriteAt(strMap2D[y, x], x, y);
                    }
                }
            }
        }
